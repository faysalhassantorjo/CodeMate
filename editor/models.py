...
class Channel(models.Model):
    ...
    channel_picture = models.ImageField(upload_to='profile_pictures/', default='https://www.piclumen.com/text-to-image/')
...
class LabTest(models.Model):
    ...
    def remaining_time(self):
        if self.end_at and self.start_at:
            ...
            end_time = (datetime.combine(date.today(), self.start_at) + timedelta(minutes=self.duration)).time()
            if now >= end_time:
                return 0
            remaining = datetime.combine(date.today(), end_time) - datetime.combine(date.today(), now)
            return int(remaining.total_seconds() // 60)  # return remaining time in minutes
...
class AttemptTest(models.Model):
    ...
    teachers_remarks = models.TextField(null=True, blank=True)
    
    @property
    def marks_obtained_percentage(self):
        if self.marks is not None and self.test.test_marks > 0:
            return (self.marks / self.test.test_marks) * 100
        return 0
