...
def test_editor(request,pk):
    ...
    try:
        ...
        container = UserContainer.objects.filter(user__user=request.user).first()
        if container:
            container = client.containers.get(container.container_id)
            status= container.status
            context.update({'container_status':status})
...
def test_evaluate(request):
    if request.method == 'POST':
        ...
        teachers_remarks = data.get('teachers_remarks','')
        
        candidate = AttemptTest.objects.get(id=candidate_id)
        candidate.marks = marks
        candidate.teachers_remarks = teachers_remarks
        candidate.save()
